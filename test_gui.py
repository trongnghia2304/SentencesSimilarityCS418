...
# 3D array to store the color of each sentence in each file
color_map = []
similarity_level = 0
...
def init_color_map():
    ...
    for i in range(len(list_similar_sentences)):
        index_file_number, index_input, compare_file_number, index_compare = list_similar_sentences[
            i]
        color_hex = color_map[0][index_file_number][index_input]
        if color_hex == []:
            color_hex = random_hex_color()
            # Store the color in the color_map for both input and compare files
            color_map[0][index_file_number][index_input] = color_hex
...
def create_text_widgets_from_sentences(list_folder, sentences_by_file, parent, list_similar_sentences=None, is_input=False):
    ...
    for i in range(len(list_folder)):
        ...
        # Sticky 'nsew' to expand in all directions
        entry_i.grid(row=i, column=0, sticky='nsew')
        ...
        for j in range(len(sentences_by_file[i])):
            ...
            if list_similar_sentences:
                if is_input:
                    color_hex = color_map[0][i][j]
                    if color_hex == []:
                        color_hex = "#000000"
                    tag_name = f"colored00{i}{j}"
                else:
...
def main():
    ...
    # Place the frames
    choose_compare_folder_frame.grid(
        row=1, column=0, padx=10, pady=10, sticky="nw")
    choose_input_folder_frame.grid(
    ...
    input_frame.grid(row=3, column=0, padx=10, pady=10,
                     sticky="nsew")  # Place input_frame
    compare_frame.grid(row=4, column=0, padx=10, pady=10,
                       sticky="nsew")  # Place compare_frame
    ...
    scrollbar = tk.Scrollbar(
        compare_frame, orient="vertical", command=compare_canvas.yview)
    scrollbar.pack(side="right", fill="y")

    # Add scrollbar to the canvas in input_frame
    scrollbar2 = tk.Scrollbar(
        input_frame, orient="vertical", command=input_canvas.yview)
    ...
    compare_canvas.create_window(
        (0, 0), window=scrollable_frame, anchor="nw", width=1000)
    ...
    input_canvas.create_window(
        (0, 0), window=scrollable_frame2, anchor="nw", width=1000)
    ...
    def on_folder_selected(compare_folder):
        if compare_folder:
            create_compare_widgets(
                scrollable_frame, list_compare_folder, sentences_by_file_compare)
        else:
            create_compare_widgets(
                scrollable_frame2, list_input_folder, sentences_by_file_input)

    def create_compare_widgets(scroll_frame, list_folder, sentences_by_file):
        for widget in scroll_frame.winfo_children():
            widget.destroy()
        create_text_widgets_from_sentences(
            list_folder, sentences_by_file, scroll_frame)
    ...
    choose_compare_folder_button = create_button_widget(
        ...
        ("Arial", 12),
        "#FFFFFF",
        lambda: choose_folder(folder_path_label, True,
                              lambda: on_folder_selected(True))
    )
    choose_compare_folder_button.grid(
        row=0, column=0, padx=10, pady=10, sticky="nw")
    ...
    choose_input_folder_button = create_button_widget(
        ...
        ("Arial", 12),
        "#FFFFFF",
        lambda: choose_folder(input_folder_path_label,
                              False, lambda: on_folder_selected(False))
    )
    choose_input_folder_button.grid(
        row=1, column=0, padx=10, pady=10, sticky="nw")

    scrollable_frame.bind(
        "<Configure>", lambda event: update_scrollregion(compare_canvas))
    scrollable_frame2.bind(
        "<Configure>", lambda event: update_scrollregion(input_canvas))

    # Label to display the compare folder path
    folder_path_label = tk.Label(
        choose_compare_folder_frame, text="Folder path: No folder selected", font=("Arial", 10), bg="#FFFFFF")
    folder_path_label.grid(row=0, column=1, padx=10, pady=10, sticky="nw")

    # Label to display the input folder path
    input_folder_path_label = tk.Label(
        choose_input_folder_frame, text="Folder path: No folder selected", font=("Arial", 10), bg="#FFFFFF")
    input_folder_path_label.grid(
        row=1, column=1, padx=10, pady=10, sticky="nw")

    # Create and place the similarity input field
    similarity_label, similarity_input = create_text_field_widget(
        similarity_frame, "Similarity level (%):")
    similarity_label.grid(row=1, column=0, padx=10, pady=2, sticky="nw")
    # To get the input value, use similarity_input.get()
    similarity_input.grid(row=1, column=1, padx=10, pady=2, sticky="nw")
    ...
    def cal_similarity():
        ...
        res = algo.find_similar_sentences_in_files(
            list_input_folder, list_compare_folder, sentences_by_file_input, sentences_by_file_compare, float(similarity_level) / 100)
        ...
        for widget in scrollable_frame2.winfo_children():
            widget.destroy()
        create_text_widgets_from_sentences(
            list_input_folder, sentences_by_file_input, scrollable_frame2, list_similar_sentences, True)
        create_text_widgets_from_sentences(
            list_compare_folder, sentences_by_file_compare, scrollable_frame, list_similar_sentences)

    # Create and place the "Compare" button
    compare_button = create_button_widget(
        similarity_frame, "Compare", ("Arial", 12), "#FFFFFF", lambda: cal_similarity())
